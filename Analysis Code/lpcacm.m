function [ak, Rn, sn] = lpcacm(fileName, start, ending)
...
% Plots linear prediction coefficients
subplot(2,2,3)
stem(ak);
...
sz = zeros(length(sn), 1);
for n = (p + 3):length(sz)  % p + 2 to begin so n - k is always > 0
    for k = 2:(p + 1)
        sz(n) = sz(n) + ak(k - 1)*sn(n - k - 1);
    end
end
